...
struct _PhoshModeManager {
  ...
  PhoshDBusKbdBacklight *kbdbacklight_proxy; /* TODO: Remove once implemented in stated */
  GCancellable                *cancel;
  gchar                       *chassis;
                                          ...
                                          PhoshModeManager *self)
{
  ...
  self->kbdbacklight_proxy = phosh_dbus_kbd_backlight_proxy_new_for_bus_finish (res, &err);
...
phosh_mode_manager_idle_init (PhoshManager *manager)
{
  ...
  /* TODO: Remove once implemented in stated */
  phosh_dbus_kbd_backlight_proxy_new_for_bus (G_BUS_TYPE_SYSTEM,
                                              G_DBUS_PROXY_FLAGS_NONE,
...
/* TODO: Remove once implemented in stated */
static void
on_kbdbacklight_set (PhoshDBusKbdBacklight *kbdbacklight_proxy,
                     GAsyncResult                *res,
                     PhoshModeManager            *self)
{
  g_autoptr (GError) err = NULL;

  if (!phosh_dbus_kbd_backlight_call_set_brightness_finish (kbdbacklight_proxy, res, &err)) {
