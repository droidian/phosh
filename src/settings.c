...
rotation_setting_clicked_cb (PhoshSettings *self)
{
  ...
  mode = phosh_rotate_info_get_mode (PHOSH_ROTATE_INFO (self->rotateinfo));
...
rotation_setting_long_pressed_cb (PhoshSettings *self)
{
  ...
  switch (mode) {
  case PHOSH_ROTATE_INFO_MODE_TOGGLE:
    mode = PHOSH_ROTATE_INFO_MODE_LOCK;
    break;
  case PHOSH_ROTATE_INFO_MODE_LOCK:
    mode = PHOSH_ROTATE_INFO_MODE_TOGGLE;
    break;
  default:
    g_assert_not_reached ();
  }
  g_debug ("Rotateifo mode: %d", mode);
  phosh_rotate_info_set_mode (PHOSH_ROTATE_INFO (self->rotateinfo), mode);
}
...
phosh_settings_class_init (PhoshSettingsClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, PhoshSettings, rotateinfo);
